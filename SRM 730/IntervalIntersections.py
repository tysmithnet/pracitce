...
def do_test(x1, y1, x2, y2, __expected):
    startTime = time.time()
    instance = IntervalIntersections()
    exception = None
    try:
        __result = instance.minLength(x1, y1, x2, y2);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("IntervalIntersections (250 Points)\n\n")
    ...
    with open("IntervalIntersections.sample", "r") as f:
        while True:
            label = f.readline()
